<div class="w-full flex flex-wrap">
    ...
    <div class="lg:w-2/3 w-full">
        ...
        <div class="flex-item border border-gray-300 rounded m-2 lg:ml-0">
            <img src="/storage/{{ $item->image->thumbnail('small', 'path') }}" alt="{{ $item->title }}" class="object-cover w-full block" alt="related post image">
            <h2 class="text-gray-800 font-medium text-2xl leading-tight p-3 pb-0"> <a href="{{ route('read', [$item->id, $item->created_at->format('dmY'), $item->slug]) }}">
                    {{ $item->title }} </a></h2>
            <div class="w-full text-xs p-3 pt-2">
                ...
                <div class="text-gray-600"> <span>{{ $item->created_at->format('d M Y, H:m') }} WIB</span> </div>
            </div>
        </div>
        @endforeach
    </div>
        ...
        <div class="p-3 pt-0 lg:px-4">
            ...
            <div class="flex item my-3 border leading-normal border-gray-300 p-3 rounded items-center">
                ...
                <div class="flex flex-col">
                    <h2 class="text-sm font-medium line-clamp-2"><a href=href="{{ route('read', [$item->id, $item->created_at->format('dmY'), $item->slug]) }}">
                        {{ $item->title }} </a></h2>
